...
go_alphazero_config = dict(
    ...
    f'data_az_ptree/go_b{board_size}-komi-{komi}_alphazero_nr-{num_res_blocks}-nc-{num_channels}_sp-mode_ns{num_simulations}_upc{update_per_collect}-mur-{model_update_ratio}_cprofile_seed0',
    env=dict(
...
if __name__ == '__main__':
    ...
    def run(max_env_step: int):
        train_alphazero([main_config, create_config], seed=0, max_env_step=max_env_step)
    import cProfile
    cProfile.run(f"run({10000})", filename="go_b9_az_sp_ptree_cprofile_ns50_10k_envstep", sort="cumulative")
