...
def parse_coord(s, board):
    """ Covert GTP-coord to (row, col) pair."""
    if s == 'pass':
...
def str_coord(loc, board):
    """ Convert katago Flattened Coordinate to GTP-coord."""
...
def katago_flatten_to_lz_flatten(loc, board):
    """ Convert katago Flattened Coordinate to lz Flattened Coordinate."""
    # self.arrsize = (board_size + 1) * (board_size + 2) + 1
...
def get_gfx_commands_for_heatmap(locs_and_values, board, normalization_div, is_percent, value_and_score_from=None,
                                 hotcold=False):
    gfx_commands = []
    divisor = 1.0
...
class KatagoPolicy:
    ...
    def get_outputs(self, gs, rules):
        with torch.no_grad():
            ...
            bin_input_data = np.zeros(shape=[1] + self.model.bin_input_shape, dtype=np.float32)
            global_input_data = np.zeros(shape=[1] + self.model.global_input_shape, dtype=np.float32)
            pla = gs.board.pla
            opp = Board.get_opp(pla)
            ...
            bin_input_data = bin_input_data.reshape([1, self.pos_len * self.pos_len, -1])
            self.features.fill_row_features(gs.board, pla, opp, gs.boards, gs.moves, move_idx, rules, bin_input_data,
                                            global_input_data, idx=0)
            bin_input_data = bin_input_data.reshape([1, self.pos_len, self.pos_len, -1])
        ...
        ownership_flat = ownership.reshape([self.features.pos_len * self.features.pos_len])
        ownership_by_loc = []
        board = gs.board
        for y in range(board.size):
            for x in range(board.size):
                loc = board.loc(x, y)
                pos = self.features.loc_to_tensor_pos(loc, board)
                if board.pla == Board.WHITE:
                    ownership_by_loc.append((loc, ownership_flat[pos]))
                else:
                    ownership_by_loc.append((loc, -ownership_flat[pos]))

        scoring_flat = scoring.reshape([self.features.pos_len * self.features.pos_len])
        scoring_by_loc = []
        board = gs.board
        for y in range(board.size):
            for x in range(board.size):
        ...
        futurepos0_flat = futurepos[0, :, :].reshape([self.features.pos_len * self.features.pos_len])
        futurepos0_by_loc = []
        board = gs.board
        for y in range(board.size):
            for x in range(board.size):
        ...
        futurepos1_flat = futurepos[1, :, :].reshape([self.features.pos_len * self.features.pos_len])
        futurepos1_by_loc = []
        board = gs.board
        for y in range(board.size):
            for x in range(board.size):
        ...
        seki_flat = seki.reshape([self.features.pos_len * self.features.pos_len])
        seki_by_loc = []
        board = gs.board
        for y in range(board.size):
            for x in range(board.size):
        ...
        seki_flat2 = seki2.reshape([self.features.pos_len * self.features.pos_len])
        seki_by_loc2 = []
        board = gs.board
        for y in range(board.size):
            for x in range(board.size):
    ...
    def get_input_feature(self, gs, rules, feature_idx):
        board = gs.board
        bin_input_data = np.zeros(shape=[1] + self.model.bin_input_shape, dtype=np.float32)
        global_input_data = np.zeros(shape=[1] + self.model.global_input_shape, dtype=np.float32)
        pla = board.pla
        opp = Board.get_opp(pla)
        move_idx = len(gs.moves)
        self.features.fill_row_features(board, pla, opp, gs.boards, gs.moves, move_idx, rules, bin_input_data,
                                        global_input_data,
                                        idx=0)
    ...
    def print_scorebelief(self, gs, outputs):
        board = gs.board
        ...
        scoredistrmid = self.pos_len * self.pos_len + EXTRA_SCORE_DISTR_RADIUS
        ret = ""
        ret += "TEXT "
        ret += "ScoreBelief: \n"
    ...
    def add_input_feature_visualizations(self, layer_name, feature_idx, normalization_div):
    ...
    def get_board_matrix_str(self, matrix, scale, formatstr):
        ret = ""
        matrix = matrix.reshape([self.features.pos_len, self.features.pos_len])
        for y in range(self.features.pos_len):
            for x in range(self.features.pos_len):
