...
def get_outputs(gs, rules):
    with torch.no_grad():
        ...
        features.fill_row_features(gs.board, pla, opp, gs.boards, gs.moves, move_idx, rules, bin_input_data,
...
def get_input_feature(gs, rules, feature_idx):
    ...
    opp = Board.get_opp(pla)
    move_idx = len(gs.moves)
    features.fill_row_features(board, pla, opp, gs.boards, gs.moves, move_idx, rules, bin_input_data, global_input_data,
                               idx=0)
...
# while True:
def katago_move(game_state, command, to_play=None):
    """
    command = ['play', 'b', 'a4']
    command = ['play', 'w', 'a4']
    ...
    if command[0] == "showboard":
        ...
        # NOTE: here bot action may be illegal action
        outputs = get_outputs(gs, rules)
        if to_play == 1:
        ...
        gfx_commands = get_gfx_commands_for_heatmap(moves_and_logprobs, gs.board, normalization_div=6, is_percent=False,
