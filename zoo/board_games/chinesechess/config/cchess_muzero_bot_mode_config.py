...
# ==============================================================
# 多GPU配置
use_multi_gpu = True  # 可根据需要开启多GPU训练
...
num_simulations = 25  # 增加到 400 以提升搜索质量,目前简单测试时，先设置为50
update_per_collect = 50
reanalyze_ratio = 0.0  # 利用MuZero重分析优势，提升样本利用率
max_env_step = int(1e8)  # 中国象棋需要更多训练步数

# UCI引擎配置（用于bot对战）
uci_engine_path = '/mnt/shared-storage-user/tangjia/chess/Pikafish/src/pikafish'  # UCI引擎完整路径
engine_depth = 4  # 引擎搜索深度，1-20，越大越强（4=中弱，10=中，15=强，20=很强）
# ==============================================================
# 配置参数结束
# ==============================================================

cchess_muzero_config = dict(
    exp_name=f'data_muzero/cchess_bot-mode_uci_depth{engine_depth}_ns{num_simulations}_seed0',
    env=dict(
        battle_mode='play_with_bot_mode',  # 关键区别1：使用 play_with_bot_mode
        channel_last=False,
        collector_env_num=collector_env_num,
        evaluator_env_num=evaluator_env_num,
        n_evaluator_episode=evaluator_env_num,
        manager=dict(shared_memory=True, ),
        # UCI引擎配置（关键区别2：添加UCI引擎配置）
        uci_engine_path=uci_engine_path,  # UCI引擎路径
        engine_depth=engine_depth,  # 引擎搜索深度，设置为 4
        render_mode=None,  # 渲染模式: 'human'打印棋盘, 'svg'生成SVG
    ),
    policy=dict(
        model=dict(
            # 15层 * 4帧 + 1层颜色 = 57层
        ...
        mcts_ctree=True,
        game_segment_length=25,  # 关键区别3：bot mode 下减半 (50 -> 25)，因为智能体只走一半的步数
        ...
        td_steps=15,  # 关键区别4：bot mode 下减半 (30 -> 15)，需要满足：game_segment_length - td_steps - num_unroll_steps > 0，即 25 - 15 - 5 = 5 > 0
