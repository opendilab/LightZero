...
from zoo.board_games.tictactoe.envs.tictactoe_mcts_bot import MCTSBot
import pytest
...
class TestTicTacToeBot:
    def test_tictactoe_two_player_mode_player0_win(self):
        ...
        env.reset()
        state = env.board
        player_0 = MCTSBot('a', 1000)    # player_index = 0, player = 1
        player_1 = MCTSBot('b', 1) # player_index = 1, player = 2
        ...
        while not env.is_game_over()[0]:
            if player_index == 0:
                action = player_0.get_actions(state, player_index=player_index, env_cfg=cfg)
                player_index = 1
            else:
                print('-'*40)
                action = player_1.get_actions(state, player_index=player_index, env_cfg=cfg)
                player_index = 0
                print('-'*40)
            env.step(action)
            state = env.board
    ...
    def test_tictactoe_two_player_mode_player1_win(self):
        ...
        env.reset()
        state = env.board
        player_0 = MCTSBot('a', 1)    # player_index = 0, player = 1
        player_1 = MCTSBot('b', 1000) # player_index = 1, player = 2
