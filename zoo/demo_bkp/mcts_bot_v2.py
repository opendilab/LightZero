...
class MCTSBot:
    ...
    def get_actions(self, state, player_index, root=None, num_simulation=None, best_action_type="UCB", step=0):
        """
        Overview:
...
# def test_tictactoe_mcts_bot_vs_rule_bot_v0_bot(num_simulations=50):
    ...
    for i in range(5):
        ...
        env.reset()
        state = env.board
        mcts_player = MCTSBot(env, 'a', num_simulations)  # player_index = 0, player = 1
        ...
        while not env.get_done_reward()[0]:
            ...
            # Set mcts_bot to be player 2.
                ...
                t1 = time.time()
                action =  mcts_player.get_actions(state, player_index=player_index)[0]
                t2 = time.time()
