...
minigrid_muzero_config = dict(
    exp_name=f'data_mz_ctree/{env_name}_muzero_ns{num_simulations}_upc{update_per_collect}_rr{reanalyze_ratio}_pelw{policy_entropy_loss_weight}_temp-final-steps-{threshold_training_steps_for_final_temperature}_collect-eps-{eps_greedy_exploration_in_collect}_rnd-rew_seed{seed}',
    env=dict(
        stop_value=int(1e6),
        env_name=env_name,
        continuous=False,
    ...
    reward_model=dict(
        ...
        latent_state_norm_clamp_max=5,
        latent_state_norm_clamp_min=-5,
...
if __name__ == "__main__":
    from lzero.entry import train_muzero_reward_model
    train_muzero_reward_model([main_config, create_config], seed=seed, max_env_step=max_env_step)