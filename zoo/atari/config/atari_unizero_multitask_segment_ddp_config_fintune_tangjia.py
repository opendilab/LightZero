...
def create_config(env_id, action_space_size, collector_env_num, evaluator_env_num, n_episode,
    ...
    return EasyDict(dict(
        ...
        policy=dict(
            multi_gpu=False,  # Disabled for single GPU training
...
def generate_configs(env_id_list, action_space_size, collector_env_num, n_episode, evaluator_env_num,
    ...
    exp_name_prefix = f'data_unizero_atari_mt_balance_20250625/atari_{len(env_id_list)}games_balance-total-stage{curriculum_stage_num}_stage-50k-20k_fix-lora-update-stablescale_vit-small-ln_moe8-lora_trans-nlayer4_brf{buffer_reanalyze_freq}_not-share-head_seed{seed}/'
...
if __name__ == "__main__":
    ...
    def get_atari_target_return_dict(ratio=1.0):
        ...
        target_scores = {
            ...
            'BreakoutNoFrameskip-v4': 30.5, # 25
            # --- 经典射击与反应 ---
            'SpaceInvadersNoFrameskip-v4': 1669.7,    
            'RiverRaidNoFrameskip-v4'   : 17117.1, 
            ...
            # --- 探索与长时序规划 (Hard-Exploration) ---
            'PitfallNoFrameskip-v4'     : 6463.7,     
            'AdventureNoFrameskip-v4'   : 0.0,        
            'EnduroNoFrameskip-v4'      : 860.5,      # 长时程任务，有昼夜变化，考验模型的耐力和持续表现
        }
    ...
    for seed in [0]:
        ...
        with DDPContext():
            train_unizero_multitask_segment_ddp(configs, seed=seed, model_path=pretrained_model_path, max_env_step=max_env_step, benchmark_name="atari",finetune_components=finetune_components)
            # ======== TODO: only for debug ========
            # train_unizero_multitask_segment_ddp(configs[:2], seed=seed, max_env_step=max_env_step) # train on the first four tasks
            
            

# TODO(pu): only for debug，设置环境变量DEBUG=1
# from train_grpo_rm_colocate import maybe_ipdb
# import torch.distributed as dist
# maybe_ipdb(dist.get_rank())