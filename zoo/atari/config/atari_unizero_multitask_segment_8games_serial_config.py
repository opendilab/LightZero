...
def generate_configs(env_id_list, action_space_size, collector_env_num, n_episode, evaluator_env_num, num_simulations, reanalyze_ratio, batch_size, num_unroll_steps, infer_context_length, norm_type, seed, buffer_reanalyze_freq, reanalyze_batch_size, reanalyze_partition, num_segments):
    """
    Generate configurations for all tasks in the environment list.
    """
    configs = []
    exp_name_prefix = f'data_unizero_mt_serial/{len(env_id_list)}games_brf{buffer_reanalyze_freq}/{len(env_id_list)}games_brf{buffer_reanalyze_freq}_1-encoder-{norm_type}-res2-channel256_gsl20_{len(env_id_list)}-pred-head_nlayer4-nh8-lsd768_mbs-320_upc160_seed{seed}/'
...
if __name__ == "__main__":
    ...
    reanalyze_ratio = 0.
    max_batch_size = 320
    batch_size = [int(min(64, max_batch_size / len(env_id_list))) for _ in env_id_list]
    num_unroll_steps = 10
