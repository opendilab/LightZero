...
script='source activate cloud-ai-lab && cd /mnt/miaohua/niuyazhe/code/LightZero && pip install -e . && pip3 install ale-py autorom && AutoROM --accept-license && cd /mnt/miaohua/niuyazhe/code/LightZero/zoo/atari/config && export NCCL_DEBUG=INFO && python -m torch.distributed.launch --nproc_per_node=2 atari_muzero_multigpu_ddp_config.py'
