...
sumtothree_cont_sampled_efficientzero_config = dict(
    exp_name=f"data_pooltool_sampled_efficientzero/vector-obs/sumtothree_vector-obs_sampled_efficientzero_k{K}_ns{num_simulations}_upc{update_per_collect}-mur{model_update_ratio}_rr{reanalyze_ratio}_sslw{ssl_loss_weight}_seed{seed}",
    env=dict(
    ...
    policy=dict(
        ...
        ssl_loss_weight=ssl_loss_weight,  # NOTE
        discount_factor=1,
        td_steps=10,
        num_unroll_steps=3,
        learning_rate=0.003,
