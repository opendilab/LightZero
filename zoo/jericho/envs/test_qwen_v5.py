...
Think step-by-step. First, analyze the current situation. Second, review your strategic memories. Third, decide on the single best action to make progress.
"""
        ...
        if not history:
            ...
            for i, h in enumerate(history):
                loc_info = ""
                # if use_structured_info and 'location' in h and (i == 0 or h['location'] != history[i-1].get('location')):
                #     loc_info = f"At '{h['location']}', "
                history_lines.append(f"{loc_info}I did: > {h['action']}\nOutcome: {h['obs']}")
            system_prompt += "\n".join(history_lines) + "\n"
        
        # system_prompt += "\n--- Current Situation ---\n"
        # if use_structured_info:
        #     system_prompt += f"[Current Location]: {location}"
        #     system_prompt += f"[My Inventory]: {inventory}\n"
        ...
        system_prompt += (
            "\n--- Your Decision ---\n"
            "Based on the situation and my memories, what is the single most logical next action to progress? "
...
if __name__ == '__main__':
    ...
    torchrun --nproc_per_node=1  /fs-computility/niuyazhe/puyuan/code/LightZero/zoo/jericho/envs/test_qwen_v5.py  
    """
    # --- 核心配置区 ---
    LLM_PROVIDER = "Qwen"
    ...
    good_trial_memory = MemoryManager(maxlen=20, similarity_threshold=SIMILARITY_THRESHOLD)
    bad_trial_memory = MemoryManager(maxlen=20, similarity_threshold=SIMILARITY_THRESHOLD)
    ...
    for episode_id in range(num_episodes):
        ...
        while not done:
            ...
            # cleaned_next_obs = llm_policy.clean_obs(next_obs_str)
            # history_entry = {'obs': cleaned_next_obs, 'action': action}
            history_entry = {'obs': next_obs_str, 'action': action}
            
            # if USE_STRUCTURED_INFO:
            #     history_entry['location'] = location_str
            ...
            # 【修改 4】: 无论 done 是否为 True，都记录下最终的观测结果
        ...
        if USE_AUTONOMOUS_REFLECTION: # 只在分数低时反思
            f.write("[Reflection Mode]: Autonomous (V2 - Focused on Final Mistake)\n")
            print("[Reflection Mode]: Autonomous (V2 - Focused on Final Mistake)")
