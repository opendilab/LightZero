...
# ============================================================================
# Model Configuration Presets (shared with Jericho/BabyAI version)
# ============================================================================
MODEL_CONFIGS = {
    "qwen2.5-0.5b": {
...
def get_priorzero_config(
    env_id: str = 'textcraft',
    seed: int = 0,
    exp_name: str = None,
    use_cot: bool = True,
    model_key: Optional[str] = "qwen2.5-3b",
    multi_gpu: bool = False,
    env_addr: str = 'http://127.0.0.1:36005',
    data_idx: int = 0,
    ...
    if exp_name is None:
        if llm_config.enable_rft:
            exp_name = (
                f"data_priorzero/textcraft/llm_rft/priorzero_dataidx{data_idx}_{model_key}_train_{llm_config.train_mode_dict.mode}/"
            ...
            exp_name = (
                f"data_priorzero/textcraft/llm_frozen/priorzero_dataidx{data_idx}_{model_key}_"
    ...
    create_config = dict(
        env=dict(
            type="textcraft",
            import_names=["zoo.textcraft.priorzero.envs.textcraft_env"],
        ),
    ...
    print(f"[Config] TextCraft configuration applied:")
    print(f"  - Model: {model_key}")
    print(f"  - Path: {llm_config.model_name_or_path}")
    print(f"  - Server: {env_addr}")
    print(f"  - data_idx: {data_idx} (selects goal item from crafting tree)")
