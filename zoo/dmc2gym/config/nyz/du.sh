...
for env in "${envs[@]}"; do
    script='source activate base && export HTTPS_PROXY=http://proxy.sensetime.com:3128/ && pip cache purge && export_LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/mnt/afs/niuyazhe/code/.mujoco/mujoco210/bin && cd /mnt/afs/niuyazhe/code/dmc2gym && pip install -e .  && pip uninstall mujoco_py -y && cd /mnt/afs/niuyazhe/code/LightZero && pip install -e . -i  https://pkg.sensetime.com/repository/pypi-proxy/simple/ && python3 -u /mnt/afs/niuyazhe/code/LightZero/zoo/dmc2gym/config/nyz/du_rz_rr1.py --env %q'
    script=${script/\%q/$env}
