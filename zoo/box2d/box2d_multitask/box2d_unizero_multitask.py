...
def create_config(env_id, observation_shape, action_space_size, collector_env_num, evaluator_env_num, n_episode, num_simulations, reanalyze_ratio, batch_size, num_unroll_steps, infer_context_length, norm_type):
...
def generate_configs(env_id_list, observation_shape_list: List, action_space_size_list: List, collector_env_num, n_episode, evaluator_env_num, num_simulations, reanalyze_ratio, batch_size, num_unroll_steps, infer_context_length, norm_type, seed):
    configs = []
    # TODO
    ...
    exp_name_prefix = f'data_unizero_mt_0826/{len(env_id_list)}games_1-encoder-{norm_type}_4-head_lsd768-nlayer2-nh8_max-bs2000_upc1000_seed{seed}/'

    for task_id, env_id in enumerate(env_id_list):
        config = create_config(
            env_id,
            observation_shape_list[task_id],
            action_space_size_list[task_id],
        ...
        configs.append([task_id, [config, create_env_manager(env_name=env_id, continuous_disc=True)]])
    return configs

def create_env_manager(env_name: str, continuous_disc: bool=False):
    if continuous_disc:
        return EasyDict(dict(
            env=dict(
                type=f'{env_name}_cont_disc',
                import_names=[f'zoo.box2d.{env_name}.envs.{env_name}_cont_disc_env'],
            ),
        ...
        return EasyDict(dict(
            env=dict(
                type=f'{env_name}',
                import_names=[f'zoo.box2d.{env_name}.envs.{env_name}_env'],
            ),
            env_manager=dict(type='subprocess'),
            policy=dict(
                type='unizero_multitask',
...
if __name__ == "__main__":
    ...
    observation_shape_list = [24, 8]
    action_space_size_list = [4, 4]
    seed = 0
    collector_env_num = 8
    n_episode = 8
    evaluator_env_num = 3
    ...
    # num_simulations = 2
    # batch_size = [4, 4, 4, 4]
    configs = generate_configs(env_id_list, observation_shape_list, action_space_size_list, collector_env_num, n_episode, evaluator_env_num, num_simulations, reanalyze_ratio, batch_size, num_unroll_steps, infer_context_length, norm_type, seed)
