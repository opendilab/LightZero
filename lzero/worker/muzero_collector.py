...
class MuZeroCollector(ISerialCollector):
    ...
    def _compute_priorities_single(self, i, agent_id, pred_values_lst, search_values_lst):
        """
        Overview:
            obtain the priorities at index i.
        Arguments:
        ...
        while True:
            ...
            for env_id, timestep in timesteps.items():
                with self._timer:
                        ...
                        if self._multi_agent:
                            for agent_id in range(agent_num):
                                game_segments[env_id][agent_id].append(
                                    actions[env_id][agent_id], to_ndarray(obs['observation'][agent_id]),
                                    reward, action_mask_dict[env_id][agent_id], to_play_dict[env_id][agent_id]
                                )
                        else:
                            game_segments[env_id].append(
                                actions[env_id], to_ndarray(obs['observation']), reward, action_mask_dict[env_id],
                    ...
                    if self._multi_agent:
                        # set game_segments size enough large in MA
                        pass
                    else:
                ...
                if timestep.done:
                    ...
                    for agent_id in range(agent_num):
                        if last_game_segments[env_id][agent_id] is not None:
                            self.pad_and_save_last_trajectory_single(
                                env_id, agent_id, last_game_segments, last_game_priorities, game_segments, dones
                            )

                        # store current segment trajectory
                        priorities = self._compute_priorities_single(env_id, agent_id, pred_values_lst, search_values_lst)
