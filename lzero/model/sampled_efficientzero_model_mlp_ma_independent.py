...
from .sampled_efficientzero_model_mlp import SampledEfficientZeroModelMLP


@MODEL_REGISTRY.register('SampledEfficientZeroModelMLPMaIndependent')
class SampledEfficientZeroModelMLPMaIndependent(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        super(SampledEfficientZeroModelMLPMaIndependent, self).__init__()
        ...
        self.global_observation_shape = global_observation_shape
        self.agent_observation_shape = agent_observation_shape
        ...
        self.obs_representation_shape = global_observation_shape + agent_observation_shape
        self.representation_network = RepresentationNetworkMLP(
            observation_shape=self.obs_representation_shape, hidden_channels=self.latent_state_dim, norm_type=norm_type
        )
    ...
    def initial_inference(self, obs: torch.Tensor) -> EZNetworkOutput:
        ...
        if isinstance(obs, dict):
            ...
            for k, v in obs.items():
                if not isinstance(v, dict):
                    batch_size = v.shape[0] * v.shape[1]    # rbatch_size=batch_size*num_agent
                    obs_device = v.device
                    break
        ...
        reward_hidden_state = (
            torch.zeros(1, batch_size,
                        self.lstm_hidden_size).to(obs_device), torch.zeros(1, batch_size,
                                                                           self.lstm_hidden_size).to(obs_device)
        )
    ...
    def _representation(self, observation: torch.Tensor) -> Tuple[torch.Tensor]:
        """
         ...
         Shapes:
            ...
            - latent_state (:obj:`torch.Tensor`): :math:`(B, H)`, where B is batch_size, H is the dimension of latent state.
         """
        obs_for_represent = torch.cat((observation['global_state'], observation['agent_state']), dim=-1)
        obs_for_represent = obs_for_represent.reshape(-1, obs_for_represent.shape[-1])
        latent_state = self.representation_network(obs_for_represent)
        if self.state_norm:
