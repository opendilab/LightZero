...
class SelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, kv_cache: Optional[KVCache] = None) -> torch.Tensor:
        ...
        def method2(q, k, v, is_training):
            ...
            manual_attn_mask = manual_attn_mask.masked_fill(manual_attn_mask != 0,
                                                            0)  # https://github.com/pytorch/pytorch/blob/main/torch/nn/functional.py#L5243
            y = torch.nn.functional.scaled_dot_product_attention(q, k, v, attn_mask=manual_attn_mask,
                                                                 dropout_p=attn_pdrop, is_causal=False)
            return y
        ...
        def method3(q, k, v, is_training):
            ...
            attn_pdrop = self.config.attn_pdrop if is_training else 0
            # 手动实现的掩码区域，将未来的时间步骤设置为负无穷
            y = torch.nn.functional.scaled_dot_product_attention(q, k, v, attn_mask=None, dropout_p=attn_pdrop,
                                                                 is_causal=True)
            return y
        ...
        for is_training in is_training_states:
            ...
            if diff_12.max() < error_tolerance:
                ...
                print(
                    f"diff_12 Outputs are not consistent for is_training={is_training}; max error is {diff_12.max().item()}")

            if diff_23.max() < error_tolerance:
                print(f"diff_23 Outputs are consistent for is_training={is_training}")
            else:
                print(
                    f"diff_23 Outputs are not consistent for is_training={is_training}; max error is {diff_23.max().item()}")
