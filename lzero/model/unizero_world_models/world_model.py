...
class FIFODefaultDict(OrderedDict):
    """
    一个具有先进先出 (FIFO) 淘汰策略的、带默认值的有序字典。

    它结合了 collections.OrderedDict, collections.defaultdict 和容量限制功能。
    """
    def __init__(self, default_factory, maxsize=0, *args, **kwargs):
        """
        初始化方法。

        Args:
            default_factory: 像 defaultdict 一样，用于为不存在的键生成默认值。
            maxsize (int): 字典的最大容量。如果为 0 或负数，则表示没有限制。
            *args, **kwargs: 传递给 OrderedDict 构造函数的其他参数。
        """
    ...
    def __missing__(self, key):
        """
        当访问的键不存在时被调用，实现 defaultdict 的功能。
        """
        # 使用 self[key] = value 的方式会触发 __setitem__，从而检查容量
        self[key] = value = self.default_factory()
        return value

    def __setitem__(self, key, value):
        """
        设置键值对，并检查是否超出容量限制以执行 FIFO 淘汰。
        """
        # 调用父类的 __setitem__ 来添加或更新元素
        super().__setitem__(key, value)

        # 如果设置了 maxsize 并且当前大小超过了限制
...
class WorldModel(nn.Module):
    ...
    def _initialize_cache_structures(self) -> None:
        ...
        #     FIFODefaultDict(dict, maxsize=self.shared_pool_size_init) 
