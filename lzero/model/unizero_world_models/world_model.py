...
class WorldModel(nn.Module):
    ...
    def reset_from_initial_observations(self, obs_act_dict: torch.FloatTensor) -> torch.FloatTensor:
        ...
        obs_embeddings = self.tokenizer.encode_to_obs_embeddings(observations,
                                                                 should_preprocess=True)  # (B, C, H, W) -> (B, K, E)

        if current_obs is not None:
            current_obs_embeddings = self.tokenizer.encode_to_obs_embeddings(current_obs,
                                                                             should_preprocess=True)  # (B, C, H, W) -> (B, K, E)
            self.latent_state = current_obs_embeddings
            outputs_wm = self.refresh_keys_values_with_initial_latent_state_for_init_infer(obs_embeddings,
                                                                                           buffer_action,
    ...
    @torch.no_grad()
    def refresh_keys_values_with_initial_latent_state_for_init_infer(self, latent_state: torch.LongTensor,
                                                                     buffer_action=None,
                    ...
                    for i in range(ready_env_num):
                        ...
                        if matched_value is not None:
                            ...
                            self.keys_values_wm_list.append(
                                copy.deepcopy(self.to_device_for_kvcache(matched_value, 'cuda')))
    ...
    def forward_recurrent_inference(self, state_action_history, simulation_index=0,
        ...
        # 输入self.keys_values_wm_list,输出为self.keys_values_wm
        self.keys_values_wm_size_list = self.trim_and_pad_kv_cache(
            is_init_infer=False)  # 与上面self.retrieve_or_generate_kvcache返回的一致
        ...
        for k in range(2):  # 假设每次只有一个动作token。
            ...
            outputs_wm = self.forward(obs_embeddings_or_act_tokens, past_keys_values=self.keys_values_wm,
                                      kvcache_independent=False, is_init_infer=False)
            # print('keys_values_wm_size_list_current:', self.keys_values_wm_size_list_current)
            self.keys_values_wm_size_list_current = [i + 1 for i in
                                                     self.keys_values_wm_size_list_current]  # NOTE: +1 ===============
            if k == 0:
        ...
        self.update_cache_context(self.latent_state, is_init_infer=False, simulation_index=simulation_index,
                                  latent_state_index_in_search_path=latent_state_index_in_search_path)  # TODO
        ...
        for i in range(latent_state.size(0)):  # 遍历每个环境
            ...
            if not is_init_infer:  # NOTE: check 在recurrent_inference时去掉前面填充的0 ============
                ...
                for layer in range(self.num_layers):
                    ...
                    k_cache_current = self.keys_values_wm._keys_values[layer]._k_cache._cache[
                        i]  # [num_heads, sequence_length, features]
                    ...
                    self.keys_values_wm_single_env._keys_values[layer]._k_cache._size = \
                        self.keys_values_wm_size_list_current[i]  # TODO
                    ...
                    if self.keys_values_wm_single_env._keys_values[layer]._k_cache._size >= context_length - 1:
                        ...
                        # 沿第3维，用0填充后3步
                        padding_size = (0, 0, 0,
                                        3)  # F.pad的参数(0, 0, 0, 3)指定了在每个维度上的填充量。这些参数是按(左, 右, 上, 下)的顺序给出的,对于三维张量来说,分别对应于(维度2左侧, 维度2右侧, 维度1左侧, 维度1右侧)的填充。
                ...
                for layer in range(self.num_layers):
                    if self.keys_values_wm._keys_values[
                        layer]._k_cache._size < context_length - 1:  # 固定只保留最近5个timestep的context
            ...
            if is_init_infer:
                ...
                self.past_keys_values_cache_init_infer_envs[i][cache_key] = copy.deepcopy(
                    self.to_device_for_kvcache(self.keys_values_wm_single_env, 'cpu'))

            else:
                self.past_keys_values_cache_recurrent_infer[cache_key] = copy.deepcopy(
                    self.to_device_for_kvcache(self.keys_values_wm_single_env, 'cpu'))
    ...
    def retrieve_or_generate_kvcache(self, latent_state, ready_env_num, simulation_index=0):
        ...
        for i in range(ready_env_num):
            ...
            if matched_value is not None:
                ...
                self.keys_values_wm_single_env = self.transformer.generate_empty_keys_values(n=1,
                                                                                             max_tokens=self.context_length)
                self.forward({'obs_embeddings': torch.from_numpy(state_single_env).unsqueeze(0).to(self.device)},
    ...
    def visualize_reconstruction_v3(self, original_images, reconstructed_images, target_predict_value, true_rewards,
                                    target_policy, predict_value, predict_rewards, predict_policy,
                                    not_plot_timesteps=[], suffix='pong', width=64):
        # 确保输入张量的维度匹配
        assert original_images.shape[0] == reconstructed_images.shape[0] // original_images.shape[1]
    ...
    def visualize_reconstruction_v2(self, original_images, reconstructed_images, target_predict_value, true_rewards,
