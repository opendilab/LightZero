...
class WorldModel(nn.Module):
    ...
    def reset_from_initial_observations(self, observations: torch.FloatTensor) -> torch.FloatTensor:
        ...
        outputs_wm = self.refresh_keys_values_with_initial_obs_tokens_for_init_infer(obs_tokens)
        self.obs_tokens = obs_tokens

        return outputs_wm, self.decode_obs_tokens(), self.obs_tokens

    @torch.no_grad()
    def refresh_keys_values_with_initial_obs_tokens_for_init_infer(self, obs_tokens: torch.LongTensor) -> torch.FloatTensor:
    ...
    fix init_infer
    本文件只是 target value在计算时使用的 init infer,使用从0 开始 reset
