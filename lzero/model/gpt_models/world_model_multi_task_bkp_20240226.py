...
class WorldModelMT(nn.Module):
    def __init__(self, obs_vocab_size: int, act_vocab_size: int, config: TransformerConfig, tokenizer, representation_network=None) -> None:
        ...
        self.act_vocab_size = 18  # for multi-task
    ...
    def forward(self, obs_embeddings_or_act_tokens, past_keys_values: Optional[KeysValues] = None,
            is_root=False, task_id=0) -> WorldModelOutput:
    ...
    def reset_from_initial_observations(self, obs_act_dict: torch.FloatTensor, task_id=0) -> torch.FloatTensor:
        ...
        outputs_wm = self.refresh_keys_values_with_initial_obs_tokens_for_init_infer(obs_embeddings, buffer_action, task_id=task_id)
    ...
    @torch.no_grad()
    # @profile
    def refresh_keys_values_with_initial_obs_tokens_for_init_infer(self, obs_tokens: torch.LongTensor, buffer_action=None, task_id=0) -> torch.FloatTensor:
        ...
        if n <= self.env_num:
            ...
            self.keys_values_wm = self.transformer.generate_empty_keys_values(n=n, max_tokens=self.config.max_tokens)
            outputs_wm = self.forward({'obs_embeddings': obs_tokens}, past_keys_values=self.keys_values_wm, is_root=False, task_id=task_id)  # Note: is_root=False
        elif n == int(256): 
    ...
    @torch.no_grad()
    # @profile
    def refresh_keys_values_with_initial_obs_tokens(self, obs_tokens: torch.LongTensor, task_id=0) -> torch.FloatTensor:
        n, num_observations_tokens, _ = obs_tokens.shape
        assert num_observations_tokens == self.num_observations_tokens
        # self.keys_values_wm = self.world_model.transformer.generate_empty_keys_values(n=n, max_tokens=self.world_model.config.max_tokens)
        self.keys_values_wm = self.transformer.generate_empty_keys_values(n=n, max_tokens=self.config.max_tokens)
        obs_embeddings_or_act_tokens = {'obs_embeddings': obs_tokens}
        outputs_wm = self.forward(obs_embeddings_or_act_tokens, past_keys_values=self.keys_values_wm, is_root=False, task_id=task_id)  # Note: is_root=False
    ...
    @torch.no_grad()
    # @profile
    def forward_initial_inference(self, obs_act_dict: torch.LongTensor, task_id=0):
        ...
        outputs_wm, obs_tokens = self.reset_from_initial_observations(obs_act_dict, task_id=task_id)
    ...
    def forward_recurrent_inference(self, state_action_history, should_predict_next_obs: bool = True, task_id=0):
        ...
        for k in range(num_passes):  # assumption that there is only one action token.
            ...
            outputs_wm = self.forward(obs_embeddings_or_act_tokens, past_keys_values=self.keys_values_wm, is_root=False, task_id=task_id)
    ...
    def compute_labels_world_model_value_policy(self, target_value: torch.Tensor, target_policy: torch.Tensor,
                                                mask_padding: torch.BoolTensor, task_id) -> Tuple[
