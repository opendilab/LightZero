...
class WorldModelMT(nn.Module):
    ...
    def forward(self, obs_embeddings_or_act_tokens, past_keys_values: Optional[KeysValues] = None,
            is_root=False, kvcache_independent=False, task_id=0) -> WorldModelOutput:
    ...
    @torch.no_grad()
    # @profile
    def reset_from_initial_observations_v2(self, obs_act_dict: torch.FloatTensor, task_id=0) -> torch.FloatTensor:
        ...
        outputs_wm = self.refresh_keys_values_with_initial_latent_state_for_init_infer_v2(obs_embeddings, buffer_action, task_id=task_id)
    ...
    @torch.no_grad()
    # @profile
    def refresh_keys_values_with_initial_latent_state_for_init_infer_v2(self, latent_state: torch.LongTensor, buffer_action=None, task_id=0) -> torch.FloatTensor:
        ...
        if n <= self.env_num:
            ...
            outputs_wm = self.forward({'obs_embeddings': latent_state}, past_keys_values=self.keys_values_wm_list, is_root=False, kvcache_independent=True, task_id=task_id) 
            ...
            outputs_wm = self.forward({'obs_embeddings': latent_state}, past_keys_values=self.keys_values_wm, is_root=False, task_id=task_id) 
            ...
            outputs_wm = self.forward({'obs_embeddings_and_act_tokens': (obs_embeddings, act_tokens)}, is_root=False, task_id=task_id) 
    ...
    def reset_from_initial_observations(self, obs_act_dict: torch.FloatTensor, task_id=0) -> torch.FloatTensor:
        ...
        outputs_wm = self.refresh_keys_values_with_initial_latent_state_for_init_infer(obs_embeddings, buffer_action, task_id=task_id)
    ...
    @torch.no_grad()
    # @profile
    def refresh_keys_values_with_initial_latent_state_for_init_infer(self, latent_state: torch.LongTensor, buffer_action=None, task_id=0) -> torch.FloatTensor:
        n, num_observations_tokens, _ = latent_state.shape
        if n <= self.env_num:
            # MCTS root节点: 需要准确的估计 value, policy_logits 或许需要结合context的kv_cache进行更准确的估计，而不是当前的从0开始推理
            self.keys_values_wm = self.transformer.generate_empty_keys_values(n=n, max_tokens=self.config.max_tokens)
            outputs_wm = self.forward({'obs_embeddings': latent_state}, past_keys_values=self.keys_values_wm, is_root=False, task_id=task_id)  # Note: is_root=False
        elif n == int(256): 
            # TODO: n=256 means train tokenizer, 不需要计算target value
            self.keys_values_wm = self.transformer.generate_empty_keys_values(n=n, max_tokens=self.config.max_tokens)
            # print('init inference: not find matched_value! reset!')
    ...
    @torch.no_grad()
    # @profile
    def refresh_keys_values_with_initial_latent_state(self, latent_state: torch.LongTensor, reset_indices=None, task_id=0) -> torch.FloatTensor:
        n, num_observations_tokens, _ = latent_state.shape
        ...
        if reset_indices is None:
            ...
            for i in reset_indices:
                ...
                outputs_wm = self.forward({'obs_embeddings': latent_state[i].unsqueeze(0).to(self.device)}, past_keys_values=self.keys_values_wm_single_env, is_root=False, kvcache_independent=False, task_id=task_id)
    ...
    @torch.no_grad()
    # @profile
    def forward_initial_inference(self, obs_act_dict: torch.LongTensor, should_predict_next_obs: bool = True, task_id=0):
        ...
        # if obs.shape[0] < 8:
        #     print('debug')
        outputs_wm, latent_state = self.reset_from_initial_observations_v2(obs_act_dict, task_id=task_id)  # TODO
    ...
    @torch.no_grad()
    # @profile
    def forward_recurrent_inference(self, state_action_history, should_predict_next_obs: bool = True, task_id=0):
        ...
        for i in range(ready_env_num):
            ...
            if matched_value is not None:
                ...
                self.keys_values_wm_single_env = self.transformer.generate_empty_keys_values(n=1, max_tokens=self.config.max_tokens)
                outputs_wm = self.forward({'obs_embeddings': torch.from_numpy(state_single_env).unsqueeze(0).to(self.device)}, past_keys_values=self.keys_values_wm_single_env, is_root=False, task_id=task_id)
        ...
        self.refresh_keys_values_with_initial_latent_state(torch.tensor(latest_state, dtype=torch.float32).to(self.device), reset_indices, task_id=task_id)
        ...
        for k in range(num_passes):  # assumption that there is only one action token.
            ...
            outputs_wm = self.forward(obs_embeddings_or_act_tokens, past_keys_values=self.keys_values_wm_list, is_root=False, kvcache_independent=True, task_id=task_id)
    ...
    def compute_loss(self, batch, target_tokenizer: Tokenizer=None,  task_id=0, **kwargs: Any) -> LossWithIntermediateLosses:
        ...
        outputs = self.forward({'obs_embeddings_and_act_tokens': (obs_embeddings, act_tokens)}, is_root=False, task_id=task_id)
