...
class WorldModel(nn.Module):
        ...
        if n <= self.env_num:
            ...
            for i in range(latent_state.size(0)):  # 遍历每个环境
                ...
                for layer in range(self.num_layers):
                    keys_values_wm_single_env._keys_values[layer]._k_cache._cache = self.keys_values_wm._keys_values[layer]._k_cache._cache[i].unsqueeze(0) # shape torch.Size([2, 100, 512])
                    keys_values_wm_single_env._keys_values[layer]._v_cache._cache = self.keys_values_wm._keys_values[layer]._v_cache._cache[i].unsqueeze(0)
                    keys_values_wm_single_env._keys_values[layer]._k_cache._size = self.keys_values_wm._keys_values[layer]._k_cache._size 
                    keys_values_wm_single_env._keys_values[layer]._v_cache._size = self.keys_values_wm._keys_values[layer]._v_cache._size
    ...
    def forward_recurrent_inference(self, state_action_history, should_predict_next_obs: bool = True):
        ...
        # 将每个环境独立的kv_cache拼接为一个batch_size为env_num的总的kv_cache
        for layer in range(self.num_layers):
            kv_cache_k_list = []
            kv_cache_v_list = []
            for keys_values in self.keys_values_wm_list:
            ...
            # self.keys_values_wm._keys_values[layer].update(torch.stack(kv_cache_k_list, dim=0).squeeze(1), torch.stack(kv_cache_v_list, dim=0).squeeze(1))
            self.keys_values_wm._keys_values[layer]._k_cache._size = max(self.keys_values_wm_size_list)  # TODO: very important
            self.keys_values_wm._keys_values[layer]._v_cache._size = max(self.keys_values_wm_size_list)
