...
class WorldModel(nn.Module):
    ...
    def compute_loss(self, batch, target_tokenizer: Tokenizer=None, **kwargs: Any) -> LossWithIntermediateLosses:
        ...
        obs_embeddings.register_hook(lambda grad: grad * 1/5)  # TODO：测试 gard scale 的作用
        ...
        if self.predict_latent_loss_type == 'mse':
            ...
            # 将 logits_observations 和 labels_observations 按照组的大小进行重塑
            # batch_size = logits_observations.shape[0]
            # group_size = 8
            # num_groups = logits_observations.shape[1] // group_size

            # logits_reshaped = logits_observations.view(batch_size, num_groups, group_size)
            # labels_reshaped = labels_observations.view(batch_size, num_groups, group_size)

            # 计算每个组内的KL散度损失,并求平均
            # loss_obs = F.kl_div(logits_reshaped.log(), labels_reshaped, reduction='none').sum(-1).mean(1)
            ...
            # 计算每个组内的KL散度损失,并求平均
            # loss_obs = F.kl_div(F.log_softmax(logits_reshaped, dim=-1), labels_reshaped, reduction='none').sum(dim=-1).mean(dim=-1) # bug
