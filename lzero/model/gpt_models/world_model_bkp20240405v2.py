...
class WorldModel(nn.Module):
    def __init__(self, obs_vocab_size: int, act_vocab_size: int, config: TransformerConfig, tokenizer) -> None:
        ...
        self.positional_embeddin_k = [self.transformer.blocks[layer].attn.key(self.pos_emb.weight).view(1, config.max_tokens, self.num_heads, self.embed_dim // self.num_heads).transpose(1, 2).detach() for layer in range(config.num_layers)]  # (B, nh, T, hs)
        self.positional_embeddin_v = [self.transformer.blocks[layer].attn.value(self.pos_emb.weight).view(1, config.max_tokens, self.num_heads, self.embed_dim // self.num_heads).transpose(1, 2).detach() for layer in range(config.num_layers)]  # (B, nh, T, hs)
        ...
        for i in range(latent_state.size(0)):  # 遍历每个环境
            ...
            for layer in range(self.num_layers):
                if self.keys_values_wm._keys_values[layer]._k_cache._size < context_length-1:  # 固定只保留最近5个timestep的context
                    ...
                    # return # TODO: 需要reset。如果上下文信息是滑动窗口（Sliding Window），需要存储不加position_embedding的kv_cache，增加了计算开销。
                    # 裁剪和填充逻辑
                    ...
                    # 计算原始位置编码和新位置编码的差值
                    original_pos_emb_k = self.positional_embedding_k[layer][:,:, 2:context_length-1, :] # self.positional_embeddin_k[layer] shape: torch.Size([1, 8, 16, 96])
                    new_pos_emb_k = self.positional_embedding_k[:,:, :context_length-3, :]
                    pos_emb_diff_k = new_pos_emb_k - original_pos_emb_k
                    original_pos_emb_v = self.positional_embedding_v[layer][:,:, 2:context_length-1, :] 
                    new_pos_emb_v = self.positional_embedding_v[:,:, :context_length-3, :]
