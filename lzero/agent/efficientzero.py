...
from lzero.agent.config.efficientzero import supported_env_cfg
...
class EfficientZeroAgent:
    ...
    .. note::
        ...
        (e.g. https://huggingface.co/OpenDILabCommunity/CartPole-v0-EfficientZero),
    ...
    def __init__(
        ...
        if env_id is not None:
            assert env_id in EfficientZeroAgent.supported_env_list, "Please use supported envs: {}".format(
            ...
            assert cfg.main_config.env.env_id in EfficientZeroAgent.supported_env_list, "Please use supported envs: {}".format(
                EfficientZeroAgent.supported_env_list
            )
        default_policy_config = EasyDict({"policy": EfficientZeroPolicy.default_config()})
        ...
        self.cfg = compile_config(
            cfg.main_config, seed=seed, env=None, auto=True, policy=EfficientZeroPolicy, create_cfg=cfg.create_config
        )
        ...
        if model is None:
            if self.cfg.policy.model.model_type == 'mlp':
                from lzero.model.efficientzero_model_mlp import EfficientZeroModelMLP
                model = EfficientZeroModelMLP(**self.cfg.policy.model)
    ...
    def train(
        ...
        replay_buffer = EfficientZeroGameBuffer(policy_config)
        collector = Collector(
            env=collector_env,
