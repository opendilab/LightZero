...
# EfficientZero
###########################################################
import lzero.rl_utils.mcts.ptree_efficientzero as tree
from ..scaling_transform import inverse_scalar_transform
from lzero.rl_utils.mcts.utils import plot_simulation_graph
...
class EfficientZeroVisualizeMCTSPtree(object):
    ...
    def search(self, roots, model, hidden_state_roots, reward_hidden_state_roots, to_play=None):
        ...
        with torch.no_grad():
            ...
            # the data storage of hidden states: storing the hidden states of all the tree root nodes
            ...
            for index_simulation in range(self.config.num_simulations):
                ...
                # traverse to select actions for each root hidden_state_index_x_lst:
                # the first index of leaf node states in hidden_state_pool, i.e. the search depth;
                # index hidden_state_index_y_lst:
                # the second index of leaf node states in hidden_state_pool, i.e. the batch root node index,
                # max is env_num, the hidden state of the leaf node is hidden_state_pool[x, y];
                # value prefix states are the same
...
# MuZero
###########################################################
import lzero.rl_utils.mcts.ptree_muzero as tree_muzero
