...
@POLICY_REGISTRY.register('muzero_multi_task_v2')
class MuZeroMTV2Policy(Policy):
    """
    Overview:
    ...
    def default_model(self) -> Tuple[str, List[str]]:
        ...
        if self._cfg.model.model_type == "conv":
            return 'MuZeroMTModel', ['lzero.model.muzero_model_multi_task']
    ...
    def _forward_learn(self, data: Tuple[torch.Tensor]) -> Dict[str, Union[float, int]]:
        ...
        policy_entropy_loss_multi_task=[]
        for task_id, data_one_task in enumerate(data):
            ...
            policy_entropy_loss_multi_task.append(policy_entropy_loss)
            ...
            consistency_loss_multi_task.append(consistency_loss)
            ...
            for step_k in range(self._cfg.num_unroll_steps):
                ...
                network_output = self._learn_model.recurrent_inference(latent_state, action_batch[:, step_k], task_id)
                latent_state, reward, value, policy_logits = mz_network_output_unpack(network_output)
                ...
                if self._cfg.model.self_supervised_learning_loss:
                    ...
                    if self._cfg.ssl_loss_weight > 0:
                        ...
                        consistency_loss_multi_task[task_id] += temp_loss
                ...
                # NOTE: the +=.
                # ==============================================================
                policy_loss_multi_task[task_id] += cross_entropy_loss(policy_logits, target_policy[:, step_k + 1])

                prob = torch.softmax(policy_logits, dim=-1)
                dist = Categorical(prob)
                policy_entropy_loss_multi_task[task_id] += -dist.entropy()

                value_loss_multi_task[task_id] += cross_entropy_loss(value, target_value_categorical[:, step_k + 1])
                reward_loss_multi_task[task_id] += cross_entropy_loss(reward, target_reward_categorical[:, step_k])
        ...
        policy_loss = sum(policy_loss_multi_task)
        value_loss = sum(value_loss_multi_task)
        reward_loss = sum(reward_loss_multi_task)
        consistency_loss = sum(consistency_loss_multi_task)
        policy_entropy_loss = sum(policy_entropy_loss_multi_task)
    ...
    def _forward_eval(self, data: torch.Tensor, action_mask: list, to_play: int = -1, ready_env_id: np.array = None, task_id: int = 0) -> Dict:
        """
        Overview:
