...
from lzero.mcts import MuZeroRNNMCTSCtree as MCTSCtree
...
class MuZeroRNNPolicy(MuZeroPolicy):
    """
    Overview:
    ...
    def default_model(self) -> Tuple[str, List[str]]:
        ...
        if self._cfg.model.model_type == "conv":
            return 'MuZeroRNNModel', ['lzero.model.muzero_rnn_model']
    ...
    def _forward_learn(self, data: torch.Tensor) -> Dict[str, Union[float, int]]:
        ...
        for step_k in range(self._cfg.num_unroll_steps):
            ...
            # `` reward`` given current ``latent_state`` ``reward_hidden_state`` and ``action``.
            ...
            latent_state, reward, reward_hidden_state, value, policy_logits = ez_network_output_unpack(
                network_output
            )
    ...
    def _forward_collect(
        ...
        with torch.no_grad():
            ...
            latent_state_roots, reward_roots, reward_hidden_state_roots, pred_values, policy_logits = ez_network_output_unpack(
                network_output
            )
