...
from lzero.rl_utils import EfficientZeroVisualizeMCTSCtree as MCTS_ctree
from lzero.rl_utils import EfficientZeroVisualizeMCTSPtree as MCTS_ptree
...
# cpp mcts
from lzero.rl_utils.mcts.ctree_efficientzero import cytree as ctree


@POLICY_REGISTRY.register('efficientzero_visualize')
class EfficientZeroVisualizePolicy(Policy):
    """
    Overview:
    ...
    def _forward_collect(
        ...
        with torch.no_grad():
            ...
            if self._cfg.mcts_ctree:
                ...
                # do MCTS for a policy (argmax in testing)
                results, leaf_hidden_states = self._mcts_collect.search(
                    roots, self._collect_model, hidden_state_roots, reward_hidden_roots, to_play
                )
