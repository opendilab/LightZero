...
class MuZeroHistoryPolicy(Policy):
    ...
    def _init_collect(self) -> None:
        ...
        if self._cfg.model.model_type in [ "conv_history"]:
            self.last_batch_obs = torch.zeros([self.collector_env_num, self._cfg.model.observation_shape[0]*self._cfg.model.history_length, 64, 64]).to(self._cfg.device)
            self.last_batch_obs_ready = self.last_batch_obs
            self.last_batch_action = [-1 for i in range(self.collector_env_num)]
    ...
    def _forward_collect(
        ...
        with torch.no_grad():
            if self._cfg.model.model_type in ["conv", "mlp"]:
                ...
                network_output = self._collect_model.initial_inference(self.last_batch_obs_ready, self.last_batch_action,
                                                                       data)
            ...
            if not self._cfg.collect_with_pure_policy:
                ...
                if self._cfg.model.model_type in ["conv_context"]:
                    ...
                    for idx, env_id in enumerate(ready_env_ids):
                        ...
                        combined_obs = torch.cat([self.last_batch_obs[env_id], data[idx]], dim=0)
                        # 仅保留最新的 total_channels 个通道
                        self.last_batch_obs[env_id] = combined_obs[-self.history_channels:]
                    
                    # 从全局历史张量中取出当前 ready 环境对应的更新后的观测
                    self.last_batch_obs_ready = self.last_batch_obs[ready_env_ids]
            else:
                for i, env_id in enumerate(ready_env_id):
    ...
    def _init_eval(self) -> None:
        ...
        if self._cfg.model.model_type in [ "conv_history"]:
            self.last_batch_obs = torch.zeros([self.evaluator_env_num, self._cfg.model.observation_shape[0]*self._cfg.model.history_length, 64, 64]).to(self._cfg.device)
            self.last_batch_obs_ready = self.last_batch_obs
    ...
    def _forward_eval(self, data: torch.Tensor, action_mask: list, to_play: int = -1,
        ...
        with torch.no_grad():
            if self._cfg.model.model_type in ["conv", "mlp"]:
                ...
                network_output = self._eval_model.initial_inference(self.last_batch_obs_ready, self.last_batch_action, data)
