...
def backpropagate(search_path, min_max_stats, to_play, value: float, discount_factor: float):
    if to_play is None or to_play == 0:
        ...
        # min_max_stats.clear()
        # root = search_path[0]
        # update_tree_q(root, min_max_stats, discount_factor, 1, to_play=0)
    else:
        # for 2 player mode
        bootstrap_value = value
        path_len = len(search_path)
        ...
        # min_max_stats.clear()
        # root = search_path[0]
        # update_tree_q(root, min_max_stats, discount_factor, 2, to_play=to_play)
...
def batch_traverse(
    roots, pb_c_base: int, pb_c_init: float, discount_factor: float, min_max_stats_lst, results: SearchResults, virtual_to_play
):
    """
    Overview:
