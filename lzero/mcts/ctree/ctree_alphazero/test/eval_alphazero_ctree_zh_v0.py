...
为了测试方便，我们使用 unittest.mock 来模拟环境（simulate_env）和策略-值函数（policy_value_func）。
"""
...
class TestMCTSAlphaZero(unittest.TestCase):
    ...
    def test_ucb_score(self):
        ...
        expected_score = expected_pb_c * child.prior_p + child.get_value()
    ...
    def test_add_exploration_noise(self):
        ...
        """
        # 为根节点添加一些子节点
        self.root.children[0] = mcts_alphazero.Node(parent=self.root, prior_p=0.4)
        self.root.children[1] = mcts_alphazero.Node(parent=self.root, prior_p=0.6)
        ...
        for action in self.root.children:
            ...
            self.assertTrue(0.4 * 0.75 <= child.prior_p <= 0.6 * 0.75 + 0.25, "探索噪声添加不正确")

    @patch.object(mcts_alphazero.MCTS, '_simulate')
    def test_get_next_action(self, mock_simulate):
        """
        测试 MCTS 的 get_next_action 方法是否正确返回动作和概率分布。
        """
        # 配置模拟环境的行为
        self.mock_env.attr = MagicMock()
        self.mock_env.attr("legal_actions").return_value = [0, 1, 2]
        self.mock_env.attr("current_player").return_value = 1
        self.mcts.simulate_env = self.mock_env

        # 执行 get_next_action
        with patch.object(self.mcts, '_expand_leaf_node', return_value=0.8), \
                patch.object(self.mcts, '_select_child', return_value=(0, self.root)):
            action, action_probs = self.mcts.get_next_action(
                state_config_for_env_reset={},  # 可以根据需要传入具体配置
            ...
            self.assertIn(action, [0, 1, 2], "返回的动作不在合法动作中")

            # 检查 action_probs 的长度是否正确
            self.assertEqual(len(action_probs), 3, "动作概率分布的长度应为 3")
    ...
    def test_expand_leaf_node(self):
        ...
        for action, prior_p in self.policy_value_func.return_value[0].items():
            self.assertIn(action, self.root.children, f"动作 {action} 的子节点应被添加")
            self.assertEqual(self.root.children[action].prior_p, prior_p, f"动作 {action} 的 prior_p 应为 {prior_p}")
