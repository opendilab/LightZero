...
cdef class Roots:
    ...
    @cython.binding
    def prepare(self, float root_noise_weight, list noises, list reward_pool,
...
@cython.binding
def batch_backpropagate(int current_latent_state_index, float discount_factor, list reward, list values, list policies,
                         MinMaxStatsList min_max_stats_lst, ResultsWrapper results, list is_reset_list,
                         list to_play_batch):
    cdef int i
    cdef vector[float] creward = reward
    cdef vector[float] cvalues = values
    cdef vector[vector[float]] cpolicies = policies

    cbatch_backpropagate(current_latent_state_index, discount_factor, creward, cvalues, cpolicies,
