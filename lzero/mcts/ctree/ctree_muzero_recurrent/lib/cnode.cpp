...
namespace tree
{
    ...
    void CNode::expand(int to_play, int current_latent_state_index, int batch_index, float reward, const std::vector<float> &policy_logits)
    {
        ...
        Arguments:
            ...
            - reward: the value prefix of the current node.
    ...
    void CRoots::prepare(float root_noise_weight, const std::vector<std::vector<float> > &noises, const std::vector<float> &reward, const std::vector<std::vector<float> > &policies, std::vector<int> &to_play_batch)
    {
        /*
        Overview:
        ...
        Arguments:
            ...
            - reward: the vector of value prefixs of each root.
            - policies: the vector of policy logits of each root.
            - to_play_batch: the vector of the player side of each root.
        */
        for (int i = 0; i < this->root_num; ++i)
        {
            this->roots[i].expand(to_play_batch[i], 0, i, reward[i], policies[i]);
            this->roots[i].add_exploration_noise(root_noise_weight, noises[i]);
            this->roots[i].visit_count += 1;
        }
    }

    void CRoots::prepare_no_noise(const std::vector<float> &reward, const std::vector<std::vector<float> > &policies, std::vector<int> &to_play_batch)
    {
        /*
        Overview:
    ...
    void cbatch_backpropagate(int current_latent_state_index, float discount_factor, const std::vector<float> &reward, const std::vector<float> &values, const std::vector<std::vector<float> > &policies, tools::CMinMaxStatsList *min_max_stats_lst, CSearchResults &results, std::vector<int> is_reset_list, std::vector<int> &to_play_batch)
    {
        /*
        Overview:
        ...
        Arguments:
            ...
            - discount_factor: the discount factor of reward.
            - reward: the value prefixs of nodes along the search path.
        ...
        */
        ...
        {
            results.nodes[i]->expand(to_play_batch[i], current_latent_state_index, i, reward[i], policies[i]);
            // reset
    ...
    int cselect_child(CNode *root, tools::CMinMaxStats &min_max_stats, int pb_c_base, float pb_c_init, float discount_factor, float mean_q, int players)
    {
        ...
        */
        ...
        for (auto a : root->legal_actions)
        {
            ...
            float temp_score = cucb_score(child, min_max_stats, mean_q, root->is_reset, root->visit_count - 1, root->reward, pb_c_base, pb_c_init, discount_factor, players);
    ...
    float cucb_score(CNode *child, tools::CMinMaxStats &min_max_stats, float parent_mean_q, int is_reset, float total_children_visit_counts, float parent_reward, float pb_c_base, float pb_c_init, float discount_factor, int players)
    {
        /*
        Overview:
        ...
        Arguments:
            ...
            - parent_reward: the value prefix of parent node.
            - pb_c_base: constants c2 in muzero.
            - pb_c_init: constants c1 in muzero.
            - disount_factor: the discount factor of reward.
            - players: the number of players.
