...
cdef extern from "lib/cnode.h" namespace "tree":
    cdef cppclass CNode:
        ...
        float reward, prior, value_sum, parent_reward

        void expand(int to_play, int current_latent_state_index, int batch_index, float reward,
    ...
    cdef cppclass CRoots:
        ...
        void prepare(float root_noise_weight, const vector[vector[float]] & noises,
                     const vector[float] & reward, const vector[vector[float]] & policies,
                     vector[int] to_play_batch)
        void prepare_no_noise(const vector[float] & reward, const vector[vector[float]] & policies,
                              vector[int] to_play_batch)
        void clear()
    ...
    void cbatch_backpropagate(int current_latent_state_index, float discount_factor, vector[float] reward,
