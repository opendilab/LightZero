...
cdef class MinMaxStatsList:
    """
    Min-max statistics list class for batch management of multiple
    min-max stats objects used for Q-value normalization
    """
    cdef CMinMaxStatsList *cmin_max_stats_lst  # Pointer to C++ min-max stats list

    def __cinit__(self, int num):
        """
        Initialize the min-max statistics list

        Arguments:
            num: Number of statistics objects (corresponding to batch size)
        """
        self.cmin_max_stats_lst = new CMinMaxStatsList(num)

    def set_delta(self, float value_delta_max):
        """
        Set the maximum value delta (used to initialize min-max range)

        Arguments:
            value_delta_max: Maximum value change
        """
        self.cmin_max_stats_lst[0].set_delta(value_delta_max)

    def __dealloc__(self):
        """
        Destructor to release memory occupied by C++ object
        """
        del self.cmin_max_stats_lst

cdef class ResultsWrapper:
    """
    Search results wrapper class for managing C++ search results object,
    containing search paths, leaf nodes, and other information
    """
    cdef CSearchResults cresults  # C++ search results object

    def __cinit__(self, int num):
        """
        Initialize the search results wrapper

        Arguments:
            num: Number of searches (batch size)
        """
