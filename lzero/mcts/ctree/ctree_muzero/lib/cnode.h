...
namespace tree {
    ...
    class CNode {
        public:
            ...
            void expand(int to_play, int simulation_index, int batch_index, float reward, const std::vector<float> &policy_logits);
    ...
    void cbatch_backpropagate(int simulation_index, float discount_factor, const std::vector<float> &rewards, const std::vector<float> &values, const std::vector<std::vector<float> > &policies, tools::CMinMaxStatsList *min_max_stats_lst, CSearchResults &results, std::vector<int> &to_play_batch);
