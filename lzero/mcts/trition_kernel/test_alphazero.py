...
def ucb_score_pytorch(
        parent_visit_count: torch.Tensor, child_visit_count: torch.Tensor, child_prior_p: torch.Tensor,
        child_value: torch.Tensor, pb_c_base: float, pb_c_init: float
) -> torch.Tensor:
...
def test_ucb_score():
    ...
    for N in BATCH_SIZES:
        ...
        reference = ucb_score_pytorch(parent_visits_cpu, child_visits_cpu, prior_p_cpu, value_cpu, PB_C_BASE, PB_C_INIT)
        ...
        for _ in range(NUM_WARMUP):
            _ = ucb_score_pytorch(parent_visits_cpu, child_visits_cpu, prior_p_cpu, value_cpu, PB_C_BASE, PB_C_INIT)
        ...
        cuda_times = []
        for _ in range(NUM_WARMUP):
            _ = ucb_score_pytorch(parent_visits, child_visits, prior_p, value, PB_C_BASE, PB_C_INIT)
        torch.cuda.synchronize()
        for _ in range(NUM_REPEATS):
