...
def select_child(node, legal_actions, pb_c_base=19652, pb_c_init=1.25):
    ...
    children_data = torch.tensor(
        [[action, id(child)] for action, child in node.children.items()], device='cuda', dtype=torch.int64
    )

    # Prepare input tensors
    legal_actions = torch.tensor(legal_actions, device='cuda', dtype=torch.int32)
    visit_counts = torch.tensor(
        [child.visit_count for child in node.children.values()], device='cuda', dtype=torch.float32
    )
    prior_probs = torch.tensor([child.prior_p for child in node.children.values()], device='cuda', dtype=torch.float32)
