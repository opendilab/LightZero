    ...
    def search(
        ...
        with torch.no_grad():
            ...
            for simulation_index in range(self._cfg.num_simulations):
                ...
                # network_output = model.recurrent_inference(state_action_history, simulation_index, latent_state_index_in_search_path, task_id=task_id)  # TODO: latent_states is not used in the model.
                network_output = model.recurrent_inference(state_action_history, task_id=task_id)  # TODO: latent_states is not used in the model.
...
class UniZeroMTMCTSCtree(object):
    """
    Overview:
        MCTSCtree for MuZero. The core ``batch_traverse`` and ``batch_backpropagate`` function is implemented in C++.
