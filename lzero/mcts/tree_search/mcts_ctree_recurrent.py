...
from lzero.mcts.ctree.ctree_muzero_recurrent import mzr_tree as tree_muzero

from lzero.policy import InverseScalarTransform, to_detach_cpu_numpy

if TYPE_CHECKING:
    from lzero.mcts.ctree.ctree_muzero_recurrent import ez_tree as ez_ctree


class MuZeroRMCTSCtree(object):
    """
    Overview:
    ...
    def roots(cls: int, active_collect_env_num: int, legal_actions: List[Any]) -> "ez_ctree.Roots":
        ...
        """
        from lzero.mcts.ctree.ctree_muzero_recurrent import mzr_tree as ctree
        return ctree.Roots(active_collect_env_num, legal_actions)

    def search(
            self, roots: Any, model: torch.nn.Module, latent_state_roots: List[Any],
            dynamics_hidden_state_roots: List[Any], to_play_batch: Union[int, List[Any]]
    ) -> None:
        """
        Overview:
        ...
        Arguments:
            ...
            - dynamics_hidden_state_roots (:obj:`list`): the value prefix hidden states in LSTM of the roots
        ...
        with torch.no_grad():
            ...
            dynamics_hidden_state_c_batch = [dynamics_hidden_state_roots[0]]
            dynamics_hidden_state_h_batch = [dynamics_hidden_state_roots[1]]
            ...
            for simulation_index in range(self._cfg.num_simulations):
                ...
                latent_states = []
                hidden_states_c_dynamics = []
                hidden_states_h_dynamics = []
                ...
                for ix, iy in zip(latent_state_index_in_search_path, latent_state_index_in_batch):
                    ...
                    hidden_states_c_dynamics.append(dynamics_hidden_state_c_batch[ix][0][iy])
                    hidden_states_h_dynamics.append(dynamics_hidden_state_h_batch[ix][0][iy])

                latent_states = torch.from_numpy(np.asarray(latent_states)).to(self._cfg.device).float()
                hidden_states_c_dynamics = torch.from_numpy(np.asarray(hidden_states_c_dynamics)).to(self._cfg.device
                                                                                                 ).unsqueeze(0)
                hidden_states_h_dynamics = torch.from_numpy(np.asarray(hidden_states_h_dynamics)).to(self._cfg.device
                                                                                                 ).unsqueeze(0)
                # .long() is only for discrete action
                last_actions = torch.from_numpy(np.asarray(last_actions)).to(self._cfg.device).long()
                """
                MCTS stage 2: Expansion
                    At the final time-step l of the simulation, the next_latent_state and reward/reward are computed by the dynamics function.
                ...
                network_output = model.recurrent_inference(
                    latent_states, (hidden_states_c_dynamics, hidden_states_h_dynamics), last_actions
                )
                ...
                network_output.dynamics_hidden_state = (
                    network_output.dynamics_hidden_state[0].detach().cpu().numpy(),
                    network_output.dynamics_hidden_state[1].detach().cpu().numpy()
                )
                ...
                reward_latent_state_batch = network_output.dynamics_hidden_state
                ...
                dynamics_hidden_state_c_batch.append(reward_latent_state_batch[0])
                dynamics_hidden_state_h_batch.append(reward_latent_state_batch[1])
