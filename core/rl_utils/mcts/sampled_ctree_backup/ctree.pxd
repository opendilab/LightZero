...
cdef extern from "cnode.h" namespace "tree":
    cdef cppclass CAction:
        CAction() except +
        CAction(vector[float] value, int is_root_action);
    ...
    cdef cppclass CNode:
        CNode() except +
        CNode(float prior, vector[CAction*] &legal_actions, int action_space_size, int num_of_sampled_actions) except +
    ...
    cdef cppclass CRoots:
        CRoots() except +
        CRoots(int root_num, vector[vector[int]] legal_actions_list, int action_space_size, int num_of_sampled_actions) except +
