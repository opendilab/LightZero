...
cdef extern from "cnode.h" namespace "tree":
    cdef cppclass CNode:
        ...
        CNode(float prior, int action_num, vector[CNode]* ptr_node_pool, vector[int] &legal_actions) except +
    ...
    cdef cppclass CRoots:
        ...
        CRoots(int root_num, int action_num, int pool_size, vector[int] &legal_actions) except +
        CRoots(int root_num, int action_num, int pool_size, vector[vector[int]] &legal_actions_list) except +
        ...
        void prepare(float root_exploration_fraction, const vector[vector[float]] &noises, const vector[float] &value_prefixs, const vector[vector[float]] &policies, vector[int] &to_play)

        void prepare_no_noise(const vector[float] &value_prefixs, const vector[vector[float]] &policies)
        void prepare_no_noise(const vector[float] &value_prefixs, const vector[vector[float]] &policies, vector[int] &to_play)
    ...
    void cbatch_back_propagate(int hidden_state_index_x, float discount, vector[float] value_prefixs, vector[float] values, vector[vector[float]] policies,
                               CMinMaxStatsList *min_max_stats_lst, CSearchResults &results, vector[int] is_reset_lst, vector[int] &to_play)
    ...
    void cbatch_traverse(CRoots *roots, int pb_c_base, float pb_c_init, float discount, CMinMaxStatsList *min_max_stats_lst, CSearchResults &results, vector[int] &virtual_to_play)
