...
@SERIAL_COLLECTOR_REGISTRY.register('episode_efficientzero_visualize')
class EfficientZeroVisualizeCollector(ISerialCollector):
    """
    Overview:
    ...
    def collect(self,
        ...
        while True:
            with self._timer:
                ...
                leaf_nodes_no_env_id = {k: v['leaf_node'] for k, v in policy_output.items()}
                leaf_hidden_states_no_env_id = {k: v['leaf_hidden_state'] for k, v in policy_output.items()}

                search_paths_no_env_id = {k: v['search_path'] for k, v in policy_output.items()}
                ...
                for index, env_id in enumerate(ready_env_id):
                    ...
                    leaf_nodes[env_id] = leaf_nodes_no_env_id.pop(index)
                    leaf_hidden_states[env_id] = leaf_hidden_states_no_env_id.pop(index)

                    search_paths[env_id] = search_paths_no_env_id.pop(index)
            ...
            for env_id, timestep in timesteps.items():
                with self._timer:
                    ...
                    if two_player_game:
                        ...
                        game_histories[env_id].append(actions[env_id], leaf_nodes[env_id], leaf_hidden_states[env_id], search_paths[env_id], to_ndarray(obs['observation']), clip_reward)
